...
/**
 * Include Structure Nested Set Adapter for ExpressionEngine
 */
...
class Structure
{
	...
	function nav()
	{
		...
		// $current_id = array_search($uri, $site_pages['uris']);
		
		// $parent_id = $this->sql->get_parent_id($current_id);
		// $entry_id = $this->EE->TMPL->fetch_param('entry_id', $current_id);
		...
		if ($start_from != '/')
		{
			...
			$found_key = array_search($this->EE->functions->remove_double_slashes('/'.html_entity_decode($start_from).'/'), $site_pages['uris']);
			if ($found_key !== FALSE)
				$branch_entry_id = $found_key;
		}
			
		$selective_data = $this->sql->get_selective_data($site_id, $current_id, $branch_entry_id, $mode, $show_depth, $max_depth, $status, $include, $exclude, $show_overview, $rename_overview);
	...
	function siblings()
	{
		...
		$pages = $this->sql->get_selective_data($site_id, $entry_id, $parent_id, 'sub', 1, -1, $status, $include, $exclude, FALSE, FALSE); // Get parent and all children
		...
		foreach($pages as $key => $page)
		{
			if (array_key_exists($entry_id, $pages) && $page['depth'] != $pages[$entry_id]['depth'])
				unset($pages[$key]);
		}
	...
	{	
		...
		$pages = $this->sql->get_selective_data($site_id, $current_id, 0, 'full', 1, -1, $status, $include, $exclude, FALSE, FALSE); // Get parent and all children		
	...
	{	
		...
		$uri = $this->EE->TMPL->fetch_param('uri', '/'.implode('/', $this->EE->uri->segment_array()).'/');
		...
		foreach ($result->result_array() as $entry)
		{
			...
			$crumbs[] = '<a href="' . $site_index . $site_pages['uris'][$entry['entry_id']] . '">' . $the_title . '</a>';
		}
	...
	function titletrail()
	{
		...
		if ($custom_titles !== FALSE)
		{
			...
			{
				$_eid = $entry['entry_id'];
				$title_array[] = isset($custom_titles[$_eid]) ? $custom_titles[$_eid] : $entry['title'];
			}
		}
		else
		{
			// just use entry titles
			foreach ($results as $entry)
	...
	{       
		$site_pages = $this->sql->get_site_pages(); // get site pages data
	...
	function first_child_redirect()
	{
		...
		if (is_numeric($entry_id))
		{
			...
			// do the redirect
			if ($first_child_uri !== FALSE) 
			{
				header("Location:" . $this->EE->functions->create_url($first_child_uri, FALSE));
				// header("Location:" . $site_pages['url'].$first_child_uri);
			}
		}
		exit();
	}
	...
	function saef_select($type = '')
	{
		...
		if ($type == 'template')
		{
			...
			$html = '<select name="structure_template_id">';
			$html .= '<option value="0">- Template -</option>';
			
			foreach ($templates as $template)
			{
				$html .= "<option value='".$template['template_id']."'>".$template['group_name'].'/'.$template['template_name']."</option>";
			}
			$html .= '</select>';
		}
		elseif ($type == 'parent')
		{
			...
			$html = '<select name="structure_parent">';
			$html .= '<option value="0">- Parent -</option>';
			
			foreach ($pages as $entry_id => $entry)
			{
				$html .= "<option value='{$entry_id}'>". str_repeat("--", $entry['depth']) . $entry['title'] ."</option>";					
			}
	...
	function set_data($data)
	{
		...
		extract($data); // channel_id, entry_id, uri, template_id, listing_cid, parent_id
		...
		if ($channel_type == 'page')
		{
			...
			if ($changed)
			{
				...
				// Update Structure 
				$this->EE->db->query("UPDATE exp_structure SET parent_id = $parent_id, listing_cid = $lcid WHERE entry_id = $entry_id");
						...
						if(isset($site_pages['uris']))
						{
							...
							if (array_key_exists($entry_id, $site_pages['uris']) && $site_pages['uris'][$entry_id] != "/")
							{
								...
								foreach ($site_pages['uris'] as $key => &$path)
								{
									...
									{
										$site_pages['uris'][$key] = str_replace($prevUri, $uri, $site_pages['uris'][$key]);
									}
								}
							}
						}
	...
	*/
	...
	function has_changed($node, $data)
	{
		...
		if ($data['entry_id'])
		{
			...
			if ($node['channel_id'])
			{
				if ($node['listing_cid'] != $data['listing_cid'])
				{
					$changed = TRUE;
				}
			}
