 ...
 * @copyright			Copyright (c) 2011 Travis Schmeisser
...
class Sql_structure
{
	 ...
	 * @return array
	 */
	function get_selective_data($site_id, $current_id, $branch_entry_id, $mode, $show_depth, $max_depth, $status, $include, $exclude, $show_overview, $rename_overview)
	{
		...
		*/
		...
		if ( ! is_array($results))
		{
			...
			$sql = "SELECT structure.*, titles.title, LOWER(titles.status) AS status
					FROM
		...
		if (count($include))
		{
			...
			if ($tree->is_of_value('entry_id', $include))
				return array(); // root node removed based on critera
			
			$tree->selective_prune('entry_id', $include);
		}
		...
		if ($show_overview == 'yes' && $mode == 'sub')
		{
			// add sql to get this entry
			$overview = $this->get_overview($branch_entry_id);
			$overview['title'] = $rename_overview; // use provided name
		...
		foreach ($results as $row)
		{
			...
			$data[$row['entry_id']] = $row;
			$data[$row['entry_id']]['uri'] = $this->EE->functions->create_page_url($pages['url'], $pages['uris'][$row['entry_id']]);
	...
	function count_segments($uri)
	{
		if ($uri != "")
		{
			$uri = $this->EE->functions->remove_double_slashes(trim(html_entity_decode($uri), '/'));
	...
	function add_attributes($pages, $entry_id, $mode)
	{
		...
		{	
			...
			if (array_key_exists($entry_id, $listing_ids) && $key == $parent_id)
			...
			// has-children class
			if ($this->EE->TMPL->fetch_param('has_children_class', FALSE) == 'yes' && $page['rgt'] - $page['lft'] > 1)
	...
	function generate_nav($selective_data, $current_id, $entry_id, $mode)
	{
		...
		for ($i=0, $foo = count($tree); $i < $foo; $i++)
		{
	...
	function create_custom_titles($include_listings = FALSE)
	{
		...
		foreach ($custom_titles as $pair)
		{
			...
			if ($result !== NULL)
				$title_fields[$result['channel_id']] = $exploded[1];
		}
		
		
		$c_fields = $this->EE->api_channel_fields->fetch_custom_channel_fields();
		$c_fields = $c_fields['custom_channel_fields'][$this->site_id];
	...
	function generate_breadcrumbs($entry_id)
	{
		$pages = $this->get_single_path($entry_id);
		
		$html = '';
		foreach($pages as $page)
		{
			$html .= $page['title'] . " &raquo; ";
		}
	...
	function get_parent_id($entry_id)
	{
	...
	function is_listing_entry($entry_id)
	{
		if (is_numeric($entry_id))
		{
			$sql = "SELECT entry_id FROM exp_structure_listings WHERE entry_id = '$entry_id' AND site_id = '$this->site_id'";
			$result = $this->EE->sql_helper->row($sql);
	 ...
	 */
	function get_structure_channels($type = '', $channel_id = '', $order = '')
	{
		$site_id = $this->EE->config->item('site_id');
		
		// Get Structure Channel Data
		$sql = "SELECT ec.channel_id, ec.channel_title, ec.site_id, esc.template_id, esc.type, esc.split_assets
	...
	function get_child_entries($parent_id)
	{
		...
		{
			$sql = "SELECT entry_id FROM exp_structure WHERE parent_id = $parent_id AND entry_id != '0' AND site_id = $this->site_id ORDER BY lft ASC";
			$results = $this->EE->db->query($sql);
	...
	{
		$site_id = $this->EE->config->item('site_id');
		$sql = "SELECT entry_id FROM exp_structure
	...
	function get_entry_titles_by_channel($channel_id)
	{
		...
		$sql = "SELECT entry_id, title FROM exp_channel_titles WHERE channel_id = $channel_id AND site_id = $this->site_id";
		$result = $this->EE->db->query($sql);
	...
	{
		$listing_channels = $this->get_structure_channels('listing');
		// print_r($listing_channels);
	}
	...
	function get_post_vals($post_fields)
	{
		$post_vals = array();
		foreach($post_fields as $postname => $notused)
		{
			$val = $this->EE->input->get_post($postname);
			if ($val !== FALSE)
				$post_vals[$postname] = $val;
		}
		return $post_vals;
	}
	 ...
	 * Create email
	 *
	 * @return integer|null site_id on success or NULL on error
	 **/
	function create_group($named_fields_array)
	{	
		$id = $this->EE->db->insert(
			'exp_structure_nav_groups',
			array_merge(
				$named_fields_array,
	...
	function get_uri()
	{
		$uri = $this->EE->functions->remove_double_slashes('/'.$this->EE->uri->uri_string().'/');
...
class structure_leaf
{
	...
	function selective_prune($key, $values, $exclude = FALSE)
	{
		...
		{
			if ($child->is_of_value($key, $values, $exclude))
			{
